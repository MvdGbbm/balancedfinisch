...
const BreathingAnimation: React.FC<BreathingAnimationProps> = ({
  ...
  const animationStyle = () => {
    ...
    return {
      ...
      boxShadow: `0 0 30px ${phase === 'inhale' ? 'rgba(56, 189, 248, 0.5)' : phase === 'hold' ? 'rgba(167, 139, 250, 0.5)' : phase === 'exhale' ? 'rgba(99, 102, 241, 0.5)' : 'rgba(45, 212, 191, 0.5)'}`
    };
  };
  const toggleActive = () => {
  ...
  return <div className="breathe-animation-container h-[450px] flex flex-col items-center justify-center">
      {/* Outer glow */}
      <div className="absolute rounded-full opacity-30 blur-xl" style={{
      width: circleSize === 'w-52 h-52' ? '220px' : '260px',
      height: circleSize === 'w-52 h-52' ? '220px' : '260px',
      background: `radial-gradient(circle, ${phase === 'inhale' ? 'rgba(56, 189, 248, 0.8)' : phase === 'hold' ? 'rgba(167, 139, 250, 0.8)' : phase === 'exhale' ? 'rgba(99, 102, 241, 0.8)' : 'rgba(45, 212, 191, 0.8)'} 0%, transparent 70%)`
    }} />
      
      {/* Main circle */}
      <div className={`${getCircleStyles()} ${circleSize}`} style={animationStyle()} onClick={toggleActive}>
        {/* Decorative rings */}
        ...
        <div className={`${getInnerCircleStyles()} ${innerCircleSize}`}>
          <div className="flex flex-col items-center justify-center text-center mx-[50px] my-[50px] rounded-lg">
            <p className="text-xl font-light mb-2 text-white drop-shadow-md">{getMessage()}</p>
            {shouldShowCounter && <p className="text-3xl font-medium text-white drop-shadow-lg">{count}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="mt-16 text-center space-y-8">
        <div className="flex justify-center">
          <button onClick={toggleActive} className="px-3 py-1 h-8 rounded-md text-sm font-medium bg-primary/40 hover:bg-primary/60 text-foreground transition-colors flex items-center shadow-md">
