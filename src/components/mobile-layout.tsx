...
export function MobileLayout({ children, showNav = true, className }: MobileLayoutProps) {
  ...
  const navItems = [
    ...
    { path: "/meditations", icon: Sunrise, label: "Meditaties" },
    { path: "/breathing", icon: Sunrise, label: "Ademhaling" },
    { path: "/planner", icon: Clock, label: "Planner" },
    { path: "/journal", icon: BookOpen, label: "Dagboek" },
    { path: "/soundscapes", icon: Music, label: "Geluiden" },
    { path: "/daily-quote", icon: Quote, label: "Quote" },
  ];
  ...
  return (
    <div className={cn("flex flex-col min-h-screen w-full bg-background", className)}>
      ...
      {showNav && (
        <div className="sticky bottom-0 z-40 w-full glass-morphism border-t">
          <nav className="flex justify-around py-2">
            {navItems.map((item) => (
              <Button
                ...
                size="sm"
                className={cn(
                  "flex flex-col items-center gap-1 rounded-lg px-2 py-1.5", 
